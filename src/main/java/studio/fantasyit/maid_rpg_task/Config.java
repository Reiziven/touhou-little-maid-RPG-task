...
public class Config {
    ...
    private static final ForgeConfigSpec.BooleanValue ENABLE_REVIVE = BUILDER
            .comment("When true, the revive task is enabled for support/master maids.")
            .define("revive.enable", true);
    ...
    private static final ForgeConfigSpec.BooleanValue ENABLE_SUPPORT_TASK = BUILDER
            .comment("When true, the Support task is available for maids to select. Default: true")
            .define("functions.support", true);
    private static final ForgeConfigSpec.BooleanValue ENABLE_MASTER = BUILDER
            .define("functions.master", false);
    private static final ForgeConfigSpec.BooleanValue ENABLE_FEED_TASK = BUILDER
            ...
            .define("functions.feed_owner", true);
    ...
    private static final ForgeConfigSpec.BooleanValue HEAL_FLAT_MODE = BUILDER
            .comment("When true, all heal amounts (support and master) are treated as flat HP values instead of percentages.\n"
                   + "Example: 0.25 becomes 0.25 HP (useless) — set your heal_amount values to e.g. 5.0 for 5 HP flat.\n"
                   + "heal_high_hp_amount is also treated as flat when this is enabled. Default: false")
            .define("heal.flat_mode", false);
    ...
    private static final ForgeConfigSpec.DoubleValue SUPPORT_HEAL_AMOUNT = BUILDER
            .comment("Percentage of max health restored by the healing circle (0.0–1.0). Default: 0.25 (25%)\n"
                   + "When heal.flat_mode=true, treated as flat HP restored instead (e.g. 5.0 = 5 HP).")
            .defineInRange("support.heal_amount", 0.25, 0.01, 100000.0);

    private static final ForgeConfigSpec.DoubleValue SUPPORT_HEAL_HIGH_HP_THRESHOLD = BUILDER
            .comment("If the target's max health is >= this value, support.heal_high_hp_amount is used instead of support.heal_amount.\n"
                   + "Set to a very large number to disable this scaling. Default: 60.0")
            .defineInRange("support.heal_high_hp_threshold", 60.0, 1.0, 100000.0);

    private static final ForgeConfigSpec.DoubleValue SUPPORT_HEAL_HIGH_HP_AMOUNT = BUILDER
            .comment("Heal percentage used when the target's max health >= support.heal_high_hp_threshold (0.0–1.0).\n"
                   + "When heal.flat_mode=true, treated as flat HP. Default: 0.20 (20%)")
            .defineInRange("support.heal_high_hp_amount", 0.20, 0.01, 100000.0);
    ...
    private static final ForgeConfigSpec.DoubleValue MASTER_HEAL_AMOUNT = BUILDER
            .comment("Percentage of max health restored per self-heal (0.0–1.0). Default: 0.20 (20%)\n"
                   + "When heal.flat_mode=true, treated as flat HP restored instead (e.g. 5.0 = 5 HP).")
            .defineInRange("master.heal_amount", 0.20, 0.01, 100000.0);

    private static final ForgeConfigSpec.DoubleValue MASTER_HEAL_HIGH_HP_THRESHOLD = BUILDER
            .comment("If the master maid's max health is >= this value, master.heal_high_hp_amount is used instead of master.heal_amount.\n"
                   + "Set to a very large number to disable this scaling. Default: 60.0")
            .defineInRange("master.heal_high_hp_threshold", 60.0, 1.0, 100000.0);

    private static final ForgeConfigSpec.DoubleValue MASTER_HEAL_HIGH_HP_AMOUNT = BUILDER
            .comment("Heal percentage used when the master maid's max health >= master.heal_high_hp_threshold (0.0–1.0).\n"
                   + "When heal.flat_mode=true, treated as flat HP. Default: 0.20 (20%)")
            .defineInRange("master.heal_high_hp_amount", 0.20, 0.01, 100000.0);
    ...
    // ---------------------------------------------------------------------------
    // Master — Sphere Shield (same mechanic as Mage's, own toggle/health/cooldown)
    // ---------------------------------------------------------------------------
    private static final ForgeConfigSpec.BooleanValue MASTER_SHIELD_ENABLED = BUILDER
            .comment("When true, the master maid activates a sphere shield when targeted in combat and will reduce maid health.\n"
                   + "The shield absorbs all damage directed at the maid until its HP is depleted.\n"
                   + "Default: true")
            .define("master.shield_enabled", true);

    private static final ForgeConfigSpec.DoubleValue MASTER_SHIELD_HEALTH = BUILDER
            .comment("HP pool of the master maid's sphere shield. Default: 30.0")
            .defineInRange("master.shield_health", 30.0, 1.0, 10000.0);

    private static final ForgeConfigSpec.IntValue MASTER_SHIELD_COOLDOWN = BUILDER
            .comment("Cooldown in ticks before the master maid's shield can reactivate after breaking.\n"
                   + "20 ticks = 1 second. Default: 600 (30 seconds)")
            .defineInRange("master.shield_cooldown", 600, 20, 144000);
    ...
    // Mage — Sphere Shield
    // ---------------------------------------------------------------------------
    private static final ForgeConfigSpec.BooleanValue MAGE_SHIELD_ENABLED = BUILDER
            .comment("When true, the mage maid activates a sphere shield when targeted in combat and will reduce maid health.\n"
                   + "The shield absorbs all damage directed at the maid until its HP is depleted.\n"
                   + "Default: true")
            .define("mage.shield_enabled", true);

    private static final ForgeConfigSpec.DoubleValue MAGE_SHIELD_HEALTH = BUILDER
            .comment("HP pool of the sphere shield. Default: 30.0")
            .defineInRange("mage.shield_health", 30.0, 1.0, 10000.0);

    private static final ForgeConfigSpec.DoubleValue MAGE_SHIELD_SCALE = BUILDER
            .comment("Scale multiplier for the sphere shield visual size relative to the maid's height.\n"
                   + "shield_diameter = maid_height * this_value. Default: 0.13")
            .defineInRange("mage.shield_scale", 0.13, 0.01, 10.0);

    private static final ForgeConfigSpec.IntValue MAGE_SHIELD_COOLDOWN = BUILDER
            .comment("Cooldown in ticks before the shield can reactivate after breaking.\n"
                   + "20 ticks = 1 second. Default: 600 (30 seconds)")
            .defineInRange("mage.shield_cooldown", 600, 20, 144000);
    ...
    // ---------------------------------------------------------------------------
    // DPS — damage / speed multipliers
    // ---------------------------------------------------------------------------
    private static final ForgeConfigSpec.DoubleValue DPS_BASE_ATTACK_BOOST = BUILDER
            .comment("Fraction added to the DPS maid's attack damage via MULTIPLY_TOTAL.\n"
                   + "0.35 = +35% damage. Halved automatically when survival_balanced=true. Default: 0.35")
            .defineInRange("dps.base_attack_boost", 0.35, 0.0, 10.0);

    private static final ForgeConfigSpec.DoubleValue DPS_OFFHAND_ATTACK_BOOST = BUILDER
            .comment("Additional MULTIPLY_TOTAL bonus when the DPS maid wields a sword or axe in the offhand.\n"
                   + "0.15 = +15% damage. Halved automatically when survival_balanced=true. Default: 0.15")
            .defineInRange("dps.offhand_attack_boost", 0.15, 0.0, 10.0);

    private static final ForgeConfigSpec.DoubleValue DPS_ATTACK_SPEED_BOOST = BUILDER
            .comment("Fraction added to the DPS maid's attack speed via MULTIPLY_TOTAL.\n"
                   + "0.30 = +30% speed. Halved automatically when survival_balanced=true. Default: 0.30")
            .defineInRange("dps.attack_speed_boost", 0.30, 0.0, 10.0);

    // ---------------------------------------------------------------------------
    // Tank — stun cooldown
    // ---------------------------------------------------------------------------
    private static final ForgeConfigSpec.IntValue TANK_STUN_COOLDOWN = BUILDER
            .comment("Cooldown in ticks between stun ability uses. 20 ticks = 1 second. Default: 200 (10s)")
            .defineInRange("tank.stun_cooldown", 200, 20, 72000);

    private static final ForgeConfigSpec.IntValue TANK_STUN_BALANCED_EXTRA_COOLDOWN = BUILDER
            .comment("Extra ticks added to the stun cooldown when survival_balanced=true. Default: 20 (+1s)")
            .defineInRange("tank.stun_balanced_extra_cooldown", 20, 0, 72000);
    ...
    public static boolean healFlatMode = false;
    public static double supportHealThreshold = 0.5;
    public static double supportHealAmount = 0.30;
    public static double supportHealHighHpThreshold = 60.0;
    public static double supportHealHighHpAmount = 0.20;
    ...
    public static double masterHealHighHpThreshold = 60.0;
    public static double masterHealHighHpAmount = 0.20;
    public static int masterHealCooldown = 400;
    public static boolean masterShieldEnabled = false;
    public static float masterShieldHealth = 30f;
    public static int masterShieldCooldown = 600;

    public static int mageSpellCooldownMin = 400;
    public static int mageSpellCooldownMax = 520;

    public static boolean mageShieldEnabled = true;
    public static float mageShieldHealth = 30f;
    public static float mageShieldScale = 0.13f;
    public static int mageShieldCooldown = 600;
    ...
    static void onLoad(final ModConfigEvent event) {
        ...
        mageShieldEnabled   = MAGE_SHIELD_ENABLED.get();
        mageShieldHealth    = MAGE_SHIELD_HEALTH.get().floatValue();
        mageShieldScale     = MAGE_SHIELD_SCALE.get().floatValue();
        mageShieldCooldown  = MAGE_SHIELD_COOLDOWN.get();
        ...
        healFlatMode           = HEAL_FLAT_MODE.get();
        supportHealThreshold   = SUPPORT_HEAL_THRESHOLD.get();
        supportHealAmount      = SUPPORT_HEAL_AMOUNT.get();
        supportHealHighHpThreshold = SUPPORT_HEAL_HIGH_HP_THRESHOLD.get();
        supportHealHighHpAmount    = SUPPORT_HEAL_HIGH_HP_AMOUNT.get();
        ...
        masterHealHighHpThreshold = MASTER_HEAL_HIGH_HP_THRESHOLD.get();
        masterHealHighHpAmount    = MASTER_HEAL_HIGH_HP_AMOUNT.get();
        masterHealCooldown   = MASTER_HEAL_COOLDOWN.get();
        masterShieldEnabled  = MASTER_SHIELD_ENABLED.get();
        masterShieldHealth   = MASTER_SHIELD_HEALTH.get().floatValue();
        masterShieldCooldown = MASTER_SHIELD_COOLDOWN.get();
