...
public class FireMagicCircleEntity extends Entity implements GeoEntity {
    ...
    private void applyBreathHit() {
        ...
        if (lockedTargetUuid != null) {
            ...
            level().getEntitiesOfClass(LivingEntity.class, wide,
                    ...
                    .forEach(t -> {
                        ...
                        if (cannotBurn(t)) spawnCursedFireParticles(t); else t.setSecondsOnFire(3);
        ...
        for (LivingEntity target : candidates) {
            if (isInBreathCone3D(target, fx, fy, fz)) {
                ...
                if (cannotBurn(target)) spawnCursedFireParticles(target); else target.setSecondsOnFire(3);
    ...
    /**
     * Returns true if the target cannot be set on fire by any means —
     * either it is inherently fire-immune, or it is currently in water/rain
     * which would immediately extinguish normal flames.
     */
    private boolean cannotBurn(LivingEntity target) {
        return target.fireImmune() || target.isInWaterOrRain();
    }
