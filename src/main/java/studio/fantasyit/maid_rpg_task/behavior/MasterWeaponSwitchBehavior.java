...
public class MasterWeaponSwitchBehavior extends Behavior<EntityMaid> {
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        if (distanceSq > RANGED_DISTANCE_THRESHOLD * RANGED_DISTANCE_THRESHOLD) {
            if (!(mainHand.getItem() instanceof ProjectileWeaponItem)) {
                if (isMeleeWeapon(mainHand)) lastMeleeWeapon = mainHand.copy();
                switchToRanged(maid);
            }
        } else {
    ...
    private void switchToRanged(EntityMaid maid) {
        ...
        for (int i = 0; i < handler.getSlots(); i++) {
            ...
            if (stack.getItem() instanceof CrossbowItem) { bestSlot = i; break; }
            else if (stack.getItem() instanceof BowItem && bestSlot == -1) bestSlot = i;
        }
        if (bestSlot != -1) swapWithMainHand(maid, bestSlot);
    }
    ...
    private void switchToMelee(EntityMaid maid) {
        ...
        if (!lastMeleeWeapon.isEmpty()) {
            for (int i = 0; i < handler.getSlots(); i++) {
                if (ItemStack.isSameItemSameComponents(handler.getStackInSlot(i), lastMeleeWeapon)) {
        ...
        for (int i = 0; i < handler.getSlots(); i++) {
            if (isMeleeWeapon(handler.getStackInSlot(i))) {
                swapWithMainHand(maid, i);
                return;
            }
        }
    ...
    private void handleOffhandRanged(EntityMaid maid) {
        ...
        if (off.isEmpty() || !(off.getItem() instanceof BowItem || off.getItem() instanceof CrossbowItem || ItemHakureiGohei.isGohei(off))) return;
        CombinedInvWrapper handler = maid.getAvailableInv(true);
        for (int i = 0; i < handler.getSlots(); i++) {
