...
public class MasterSelfHealBehavior extends Behavior<EntityMaid> {
    ...
    protected void start(ServerLevel level, EntityMaid maid, long gameTime) {
        double healPct = maid.getMaxHealth() >= Config.masterHealHighHpThreshold
                ? Config.masterHealHighHpAmount : Config.masterHealAmount;
        float heal = Config.healFlatMode
                ? (float) healPct
                : maid.getMaxHealth() * (float) healPct;
