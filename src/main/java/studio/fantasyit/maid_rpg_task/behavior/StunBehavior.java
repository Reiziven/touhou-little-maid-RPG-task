...
public class StunBehavior extends Behavior<EntityMaid> {
    private static final int STUN_DURATION = 70;
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, EntityMaid maid) {
        ...
        return offhand.canPerformAction(ItemAbilities.SHIELD_BLOCK) && (level.getGameTime() - lastStunTime) >= cooldown && !stunned;
    }
    ...
    protected void start(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        for (Mob mob : allMobs) {
            mob.addEffect(new MobEffectInstance(EffectRegistry.DAMAGE_DECREASE, STUN_DURATION, Config.tankStunDamageDecreaseLevel, false, true));
            mob.addEffect(new MobEffectInstance(net.minecraft.world.effect.MobEffects.MOVEMENT_SLOWDOWN, STUN_DURATION, 254, false, true));
            mob.addEffect(new MobEffectInstance(net.minecraft.world.effect.MobEffects.DIG_SLOWDOWN, STUN_DURATION, 254, false, true));
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        for (Mob mob : mobs) mob.setTarget(null);
        stunned = false;
    }

    @Override
    protected boolean canStillUse(ServerLevel level, EntityMaid maid, long gameTime) { return stunned; }

    @Override
    protected void stop(ServerLevel level, EntityMaid maid, long gameTime) { stunned = false; }
}
