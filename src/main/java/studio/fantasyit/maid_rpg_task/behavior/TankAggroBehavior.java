...
public class TankAggroBehavior extends Behavior<EntityMaid> {
    ...
    private static class AggroMaidGoal extends TargetGoal {
        ...
        public boolean canUse() {
            return maid.isAlive() && maid.level() == mob.level() && !mob.isDeadOrDying();
        }
        ...
        @Override
        public void stop() {
            if (mob.getTarget() == maid) mob.setTarget(null);
        }
    }
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        if (gameTime % 20 == 0) redirectAggro(level, maid);
    }
    ...
    private void redirectAggro(ServerLevel level, EntityMaid maid) {
        ...
        List<Mob> mobs = level.getEntitiesOfClass(Mob.class, area, mob -> mob.isAlive() && !mob.isNoAi());
        for (Mob mob : mobs) {
